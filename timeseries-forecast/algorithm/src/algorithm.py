...
def load_data(algorithm: Algorithm) -> pd.DataFrame:
    _, filepath = next(algorithm.job_details.next_path())
...
def save(result: ResultType, base: Path, algorithm: Algorithm) -> None:
    if not result:
        return
    ...
    store(
        base / "model.pkl",
        lambda f: cloudpickle.dump(result.pipeline, f),
    )
    store(
        base / "scores.csv",
        lambda f: pd.DataFrame(result.metrics).to_csv(f),
    )
