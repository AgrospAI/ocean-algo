...
In older versions, when publishing an _algorithm_ you would have to download the `entrypoint.py` file from this repository and run it from there. Now, you can just use the `ocean-execute` script from the `ocean-runner` library, specifying the module to execute (`default="algorithm.src.algorithm"`).
