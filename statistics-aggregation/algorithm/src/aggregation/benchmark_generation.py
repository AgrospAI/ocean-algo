...
class BenchmarkReference:
    ...
    def generate_benchmark_reference(self, df: pd.DataFrame) -> dict:
        """
        ...
        df["province"] = (
            ...
            .astype(str)
            .str[:2]
            .map(self.config.get("postal_code", {}))
            .fillna("Desconocido")
        )
        ...
        for grouped in groups_combinations:
            reference_data.update(self._calculate_reference_by_group(grouped))

        return reference_data

    def _calculate_reference_by_group(self, grouped) -> dict:
        """
        ...
        section_map = self.config.get("section_mapping", {})
        ...
        for (x, y), group in grouped:
            ...
            survey_schema = self.config.get("survey_schema", {})

            for col in group.columns:
                if col in score_cols:
                    score_stats = self._calculate_score_stats(group[col])
                    stats["scores"][col] = score_stats

                elif col in adoption_cols:
                    adoptation_rate_stats = self._calculate_adoption_rate(
                        group[col], keyword="Sí|En producción"
                    )
                    stats["adoption_rates"][col] = adoptation_rate_stats
                    stats["all_questions"][section_map.get(col, "Unknown")][col] = (
                        adoptation_rate_stats
                    )

                elif col in market_cols:
                    stats["market_leaders"][col] = self._format_top_items(group[col])
                    stats["all_questions"][section_map.get(col, "Unknown")][col] = (
                        self._format_top_items(group[col])
                    )
            ...
            if "it_outsourcing_level" in group.columns:
                numeric_vals = (
                    group["it_outsourcing_level"].map(outsourcing_map).dropna()
                )
    ...
    def _calculate_score_stats(self, series):
    ...
    def _calculate_adoption_rate(self, series, keyword):
    ...
    def _format_top_items(self, series, n=3):
